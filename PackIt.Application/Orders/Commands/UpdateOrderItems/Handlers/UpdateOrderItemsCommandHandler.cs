...
{
    internal class UpdateOrderItemsCommandHandler : ICommandHandler<UpdateOrderItemsCommand>
    {
        private readonly IOrderRepository _orderRepository;
        private readonly IUnitOfWork _unitOfWork;
        private readonly IOrderItemsFactory _orderItemsFactory;

        public UpdateOrderItemsCommandHandler(IOrderRepository orderRepository, IOrderItemsFactory orderItemsFactory,  IUnitOfWork unitOfWork)
        {
            _orderRepository = orderRepository;
            _unitOfWork = unitOfWork;
            _orderItemsFactory = orderItemsFactory;
        }
        public async Task HandleAsync(UpdateOrderItemsCommand command, CancellationToken cancellationToken)
        {
            ...
            if (order is null)
                throw new OrderNotFoundException($"Oeder with Id: {OrderId} was not found.");
