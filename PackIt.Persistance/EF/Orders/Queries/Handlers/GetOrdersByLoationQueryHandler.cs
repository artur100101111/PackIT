...
{
    internal class GetOrdersByLoationQueryHandler: IQueryHandler<GetOrdersByRequestedLocationQuery,  IEnumerable<OrderDto>>
    {
        private DbSet<OrderReadModel> _orders;
        private IMapper _mapper;
        private IDateTimeService _dateTimeService;

        public GetOrdersByLoationQueryHandler(ReadDbContext readDbContext, IMapper mapper, IDateTimeService dateTimeService)
        {
            _orders = readDbContext.Orders;
            _mapper = mapper;
            _dateTimeService = dateTimeService;
        }
        public async Task<IEnumerable<OrderDto>> HandleAsync(GetOrdersByRequestedLocationQuery query, CancellationToken cancellationToken)
        {
            var orders = await _orders.Where(o => o.RequestedDeliveryLocation.Code == query.LocationCode)
